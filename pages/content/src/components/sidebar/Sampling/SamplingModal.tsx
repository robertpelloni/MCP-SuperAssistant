      ...
      <Card className="w-full max-w-lg bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-xl max-h-[80vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
        ...
        <CardContent className="p-4 flex-1 overflow-y-auto space-y-4">
          <div className="space-y-2">
            <Typography variant="subtitle" className="font-medium text-slate-700 dark:text-slate-300">
              Context Messages:
            </Typography>
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded p-2 text-sm max-h-40 overflow-y-auto space-y-2 border border-slate-200 dark:border-slate-700">
              {messages.map((msg: any, i: number) => (
                <div
                  key={i}
                  className="flex flex-col border-b border-slate-100 dark:border-slate-700/50 last:border-0 pb-2 mb-2 last:pb-0 last:mb-0">
                  <span className="text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-1">
                    {msg.role}
                  </span>
          ...
          <div className="space-y-2">
            ...
            />
            {request.maxTokens && (
              <div className="text-xs text-slate-400 text-right">Max Tokens: {request.maxTokens}</div>
            )}
          </div>
        </CardContent>
