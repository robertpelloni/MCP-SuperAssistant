...
export const useMcpCommunication = () => {
  ...
  const refreshTools = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        const validatedTools = updated.filter(tool => tool && typeof tool.name === 'string' && tool.name.length > 0);
  ...
  const updateServerConfig = useCallback(
    async (cfg: Partial<ServerConfig>) => {
      if (!isInitialized) {
        throw new Error('Communication layer not initialized');
      }
  ...
  const normalizedTools = useMemo(() => {
    ...
    if (enabledTools.length > 0) {
      logMessage(
        `[useMcpCommunication] Tools normalized and filtered: ${enabledTools.length}/${normalized.length} enabled`,
      );
    }
  ...
  useEffect(() => {
    ...
    if (toolCount > 0) {
      logMessage(
        `[useMcpCommunication] Available tool names: ${normalizedTools
          .map(t => t.name)
          .slice(0, 3)
  ...
  useEffect(() => {
    if (normalizedTools.length > 0) {
      ...
      const toolsWithSchema = normalizedTools.filter(
        tool =>
