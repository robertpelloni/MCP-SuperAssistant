...
export const useMemoryActions = () => {
  ...
  const getAdapter = useCallback((type: 'vector' | 'anything' | 'local') => {
    switch (type) {
      case 'vector':
        return new VectorAdapter({ saveToolName: vectorSaveTool, searchToolName: vectorSearchTool });
      case 'anything':
        return new AnythingLLMAdapter({ baseUrl: anythingLlmBaseUrl, apiKey: anythingLlmApiKey, workspaceSlug: 'default' });
      case 'local':
  ...
  const saveContent = useCallback(async (type: 'vector' | 'anything' | 'local', content: string, title: string) => {
    if (!content) return;
    setIsSaving(true);
    try {
  ...
  const searchMemory = useCallback(async (type: 'vector' | 'anything' | 'local', query: string) => {
    if (!query) return;
    setIsSearching(true);
    try {
      const adapter = getAdapter(type);
