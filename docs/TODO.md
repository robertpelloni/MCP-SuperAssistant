...
## Immediate Polish
- [ ] **Console Cleanup**: Remove debug `console.log` statements in production builds (verify `logger` config).
...
## UX Improvements
- [ ] **Context Manager**: Add ability to "Pin" contexts to the top of the list.
- [ ] **Macro Builder**: Add "Test Step" button to run a single step during creation.
- [ ] **System Info**: Add a dynamic "Reload Extension" button for developers.

## Future Features (Phase 5)
- [ ] **Multi-Modal Support**: Add image drag-and-drop to Input Area (handled by MCP `image` resource).
